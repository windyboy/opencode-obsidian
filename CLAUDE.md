...
**TodoManager** (`src/todo/todo-manager.ts`): Task planning and orchestration system that extracts tasks from conversations, creates TaskPlan structures, tracks progress, and persists to vault.

**MCPManager** (`src/mcp/mcp-manager.ts`): Model Context Protocol integration for connecting to MCP servers and accessing their tools and resources.
...
**TodoManager** (`src/todo/todo-manager.ts`): Task planning and orchestration system that creates TaskPlan structures, tracks progress, and persists to vault.

**AgentOrchestrator** (`src/orchestrator/agent-orchestrator.ts`): Agent loop state machine that coordinates task planning and execution with context retrieval and step tracking.

**MCPManager** (`src/mcp/mcp-manager.ts`): Model Context Protocol integration for connecting to MCP servers, managing tools and resources, with stdio transport support.

### Known Architecture Issues

The ARCHITECTURE.md file documents several known issues that have been addressed in recent updates:

**Recently Completed**:

1. **HTTP Client SDK Migration** - Migrated from custom HTTP client to `@opencode-ai/sdk/client` for standardized API interactions
2. **Task Orchestration Enhancement** - Upgraded TodoManager to full task orchestration system with planning and execution tracking
3. **MCP Integration** - Implemented Model Context Protocol support for extended tool capabilities
...
See `docs/ARCHITECTURE.md` for detailed improvement plans and implementation roadmap.
