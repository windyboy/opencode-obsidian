...
export class ErrorHandler {
	private config: Required<
		Omit<ErrorHandlerConfig, "notificationCallback">
	> & {
		notificationCallback?: NotificationCallback;
	};
	...
	handleError(
		...
		if (
			this.config.showUserNotifications &&
	...
	private notifyUser(
		...
		// Don't add prefix for already-enhanced user-friendly error messages
		// Enhanced messages typically contain phrases like "Unable to connect", "Failed to", etc.
		const isAlreadyEnhanced =
			error.message.includes("Unable to connect") ||
			error.message.includes("Please ensure") ||
			error.message.startsWith("Failed to");

		// Only add context prefix if the error message is not already user-friendly
